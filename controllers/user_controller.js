...
exports.index = function(req,res, next){
    ...
    search.replace(/ /g,'%');
    search = "%"+search+"%";
    models.User.findAll({where: {username: {$like: search}}, order:['username']}).then(function(users){
...
exports.show = function (req, res, next){
    models.Album.findAll({where:{UserId: req.user.id}}).then(function(item){
      res.render('users/show', {user: req.user, albums: item});
    });
};
	...
	.then(function(existing_user){
	   if(existing_user){
		...
		return user.save({fields: ["username", "password", "salt"]})
          ...
          var album = {name: "My First Album", isPrivate: "false", UserId: user.id};
